...
export function BilibiliImport({ catalog, onConfirm }: { catalog: Anime[]; onConfirm: (anime: Anime[]) => void }) {
  ...
  return (
    <div className="bili-import">
      <div className="bili-import-heading"><div><p className="eyebrow">BILIBILI LOCAL IMPORT</p><h3>B站片单文件导入</h3><p>CSV/JSON 在浏览器本地解析。本站不接收密码、Cookie、SESSDATA 或观看记录原文。</p></div><label className="file-button">选择 CSV / JSON<input aria-label="选择B站片单文件" type="file" accept=".csv,.json,text/csv,application/json" onChange={readFile} /></label></div>
